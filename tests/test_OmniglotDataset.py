...
from src.data.OmniglotDataset import OmniglotDataset, download_dataset_omniglot
...
class TestOmniglotDataset(TestCase):
    ...
    def test_download_dataset(self):
        ...
        assert len(os.listdir(train_dir)) == 1032

        assert os.path.exists(val_dir)
        assert len(os.listdir(val_dir)) == 172

        assert os.path.exists(test_dir)
        assert len(os.listdir(test_dir)) == 464